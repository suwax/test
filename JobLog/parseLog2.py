...
class Job(object):
	...
	def get_mapruntime(self):
		...
		map_slot = []
		for t in self.map_tasks.values():
			duration_list.append((t.finish_time - t.start_time) / 1000)
		
		duration_list.sort(None, None, True)
		...
		for d in duration_list:
			if map_index < 40:
				...
				min_mapload = min(map_slot)
				min_mapload_index = map_slot.index(min_mapload)
				new_mapload = min_mapload + d
				map_slot[min_mapload_index] = new_mapload

		return max(map_slot)
	
	def get_reducerTotalRunTime(self):
		return self.get_shuffleruntime() + self.get_reduceruntime()
		
	def get_totalruntime(self):
		return self.get_mapruntime() + self.get_reduceruntime()
	...
	def get_reduceruntime(self):
		...
		for d in duration_list:
			if reduce_index < 20:
				reduce_slot.append(d)
				reduce_index = reduce_index + 1
			else:
				min_reduceload = min(reduce_slot)
				min_reduceload_index = reduce_slot.index(min_reduceload)
				new_reduceload = min_reduceload + d
				reduce_slot[min_reduceload_index] = new_reduceload
		
		if not reduce_slot:
			return 0
		return max(reduce_slot)		

class Task(object):
	def __init__(self, tid, start_time, finish_time, counter):
...
# 	print tid,task_type,is_map
	...
	task = Task(tid, None, None, None) if is_new else tasks[tid]
...
def main():
	...
	for i in range(1, len(sys.argv)):
		...
		print sys.argv[i] + "\t" + str(job.get_mapruntime()) + "\t" + str(job.get_reduceruntime()) + "\t" + str(job.get_totalruntime())
